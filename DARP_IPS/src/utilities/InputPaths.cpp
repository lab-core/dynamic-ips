...
void InputPaths::initializeOutputs(const std::string &algorithm, const std::string &solutionMode, bool saveScratch,
    ...
    if (saveScratch) {
        ...
        if (check == -1)
            throw myTools::myException("Output directory can not be created!!!", __FILE__,__LINE__);
        outputDir_ = outputFolder + "/" + solutionMode + "_" + algorithm + "_" + resultFolder + "_" +
                     std::to_string(nbVehicles) + "/";
    } else {
        time_t now = time(nullptr);
        tm *curr_tm = localtime(&now);
        ...
        outputDir_ = instanceDir_ + solutionMode + "_" + algorithm + "_" + resultFolder + "_" +
                     std::to_string(nbVehicles) + "/";
    }
...
void InputPaths::makeInstanceOutput(const std::string& instNum) {
    ...
    std::string folder_name = dataDir_ + instanceFolder_+"_Epoch";
