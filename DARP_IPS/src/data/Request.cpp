...
void Request::setPenalty(int epoch, PParameters &parameters, float simulationStart) {
    penalty_ = static_cast<float>(parameters->deltaPram_ * pow(2, (static_cast<float>(parameters->epochLength_ * epoch) - (earlyPick_-simulationStart)) / static_cast<float>(10 * parameters->epochLength_)));
}
