...
void MIPSolver(PInstance& PInst)
{
    ...
    for (int v = 0; v < PInst->nbVehicles_; ++v) {
        U[v] = IloNumVarArray (env, PInst->mainGraph_->nbNodes_, 0, IloInfinity, ILOFLOAT);
        W[v] = IloNumVarArray (env, PInst->mainGraph_->nbNodes_, 0, IloInfinity, ILOFLOAT);
        X[v] = IloNumVar2D(env, PInst->mainGraph_->nbNodes_);
        for (int i = 0; i < PInst->mainGraph_->nbNodes_; ++i) {
            X[v][i] = IloNumVarArray(env, PInst->mainGraph_->nbNodes_, 0.0, 1.0, ILOBOOL);
        }
    }
    ...
    for (int i = 0; i < PInst->nbRequests_; ++i) {
        ...
        for (int v = 0; v < PInst->nbVehicles_; ++v) {
            int nodeIndex = PInst->mainGraph_->nodeIDToInt_[Tools::createNodeID(PInst->requests_[i]->getRequestId(), PICKUP)];
    ...
    for (int i = 0; i < PInst->nbRequests_; ++i) {
        ...
        int nodeIndex = PInst->mainGraph_->nodeIDToInt_[nodeID];

        IloExpr expr2(env);
        expr2 += Z[i];
        for (int v = 0; v < PInst->nbVehicles_; ++v) {
            for (int j = 0; j < PInst->mainGraph_->nbNodes_; ++j) {
                if (PInst->mainGraph_->nodes_[PInst->mainGraph_->intToNodeID_[j]]->type_ != SOURCE)
    ...
    for (int v = 0; v < PInst->nbVehicles_; ++v) {
        ...
        IloExpr expr5(env);
        for (int j = 0; j < PInst->mainGraph_->nbNodes_; ++j) {
            if (PInst->mainGraph_->nodes_[PInst->mainGraph_->intToNodeID_[j]]->type_ != SINK)
                expr5 += X[v][j][1];
        }
        ...
        for (int i = 0; i < PInst->nbRequests_; ++i) {
            ...
            int pickIndex = PInst->mainGraph_->nodeIDToInt_[pickID];

            std::string dropID = Tools::createNodeID(PInst->requests_[i]->getRequestId(), DROPOFF);
            int dropIndex = PInst->mainGraph_->nodeIDToInt_[dropID];
            ...
            float t = calcTravelTime(PInst->mainGraph_->nodes_[pickID],
                                     PInst->mainGraph_->nodes_[dropID]);
        ...
        for (int i = 0; i < PInst->mainGraph_->nbNodes_; ++i) {
            ...
            // constraints 3a -------------------
            IloExpr expr3(env);
            if ((PInst->mainGraph_->nodes_[PInst->mainGraph_->intToNodeID_[i]]->type_ == PICKUP) ||
                (PInst->mainGraph_->nodes_[PInst->mainGraph_->intToNodeID_[i]]->type_ == DROPOFF)) {
                for (int j = 0; j < PInst->mainGraph_->nbNodes_; ++j) {
                    if (PInst->mainGraph_->nodes_[PInst->mainGraph_->intToNodeID_[j]]->type_ != SOURCE)
                        expr3 += X[v][i][j];
                }
            ...
            for (int j = 0; j < PInst->mainGraph_->nbNodes_; ++j) {
                ...
                // constraints 8a -------------------
                IloExpr expr8(env);
                expr8 = U[v][i] + PInst->mainGraph_->nodes_[PInst->mainGraph_->intToNodeID_[i]]->deltaTime_
                        + calcTravelTime(PInst->mainGraph_->nodes_[PInst->mainGraph_->intToNodeID_[i]],
                                         PInst->mainGraph_->nodes_[PInst->mainGraph_->intToNodeID_[j]])
                        - U[v][j] - 7200 * (1 - X[v][i][j]);
                MIPModel.add(expr8 <= 0);

                // constraints 14a -------------------
                IloExpr expr14(env);
                expr14 = W[v][i] + PInst->mainGraph_->nodes_[PInst->mainGraph_->intToNodeID_[j]]->nbPassengers_
                         - W[v][j] - 100 * (1 - X[v][i][j]);
            ...
            /*if (PInst->mainGraph_->nodes_[PInst->mainGraph_->intToNodeID_[i]]->nodeStatus_ == PLANNED) {
                std::string onboardID = PInst->mainGraph_->intToNodeID_[i];
                std::string pickNodeID = PInst->mainGraph_->nodes_[onboardID]->pairNodeID_;

                // constraints 7a -------------------
                IloExpr expr7(env);
                for (int j = 0; j < PInst->mainGraph_->nbNodes_; ++j) {
                    expr7 += X[v][j][PInst->mainGraph_->nodeIDToInt_[onboardID]];
                }
                MIPModel.add(expr7 == 1);

                // constraints 13a -------------------
                IloExpr expr13(env);
                expr13 = U[v][PInst->mainGraph_->nodeIDToInt_[onboardID]] - PInst->mainGraph_->nodes_[pickNodeID]->reachTime_
                         - PInst->mainGraph_->nodes_[pickNodeID]->deltaTime_;

                float t = calcTravelTime(PInst->mainGraph_->nodes_[onboardID],
                                         PInst->mainGraph_->nodes_[pickNodeID]);
