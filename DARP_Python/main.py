...
def main(name):
    ...
    for file in glob.glob(c.VEHICLES_DIR + "*.json"):
        ...
        uf.create_resticted_vehicle_dataset(Path(file).stem, selected_districts, manhattan_districts)
...
#            uf.create_vehicle_dataset_zone(Path(file).stem, districts=manhattan_districts.districts, nb_per_zone=day_dataset.nb_vehicle_per_district)
        ...
        vf.plot_map_request_cells(district_network=manhattan_districts, dataset=day_dataset, print_id=False,
                                  file_name=day_dataset.file_name)
